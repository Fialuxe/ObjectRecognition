...
function visualize_mistakes(imageStore, YTest, YPred, testIdx, methodTag)
    ...
    % --- ここを修正 ---
    currentTestFiles = imageStore.Files(testIdx); % 1. まずテストセットのファイル群を抽出
    wrongFiles = currentTestFiles(wrongIdx);      % 2. その中から誤分類されたものを抽出
    % -----------------
    ...
    for k = 1:length(wrongFiles)
        try
            img = ensure_rgb(imread(wrongFiles{k}));
            thumbnails{k} = img;
        catch
